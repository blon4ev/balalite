...
SUIT_CHIP_JOKERS: List[Joker] = [
    Joker(
        ...
        f"공격에 사용한 {suit.symbol} 카드 1장당 +8 데미지", 5, "add", _suit_chip_bonus(suit, 8), "common",
    )
    for suit in Suit
]
...
NO_SUIT_JOKERS: List[Joker] = [
    Joker(
        ...
        f"이번 공격에 {suit.symbol} 카드가 없으면 +6 배율", 5, "add", _no_suit_mult_bonus(suit, 6), "common",
    )
    for suit in Suit
]

RANK_PRESENT_JOKERS: List[Joker] = [
    Joker("queen_grace", "여왕의 은총", "판정에 Q가 포함되면 +10 데미지", 5, "add",
          _rank_present_bonus(Rank.QUEEN, 10, "chips"), "common"),
    Joker("jack_wit", "잭의 재치", "판정에 J가 포함되면 +6 배율", 5, "add",
          _rank_present_bonus(Rank.JACK, 6, "mult"), "common"),
    Joker("ten_treasure", "텐의 보물", "판정에 10이 포함되면 +12 데미지", 5, "add",
...
HAND_TYPE_SCHOLAR_JOKERS: List[Joker] = [
    Joker(
        ...
        f"{ht.label}로 낼 때, 그 콤보의 교범 레벨 1당 +2 배율",
...
ENHANCEMENT_COUNT_JOKERS: List[Joker] = [
    Joker("bonus_appraiser", "보너스 감정사", "판정에 포함된 보너스 강화 카드 1장당 +6 배율", 6, "add",
          _enhancement_count_bonus("bonus", 6, "mult"), "uncommon"),
    Joker("mult_appraiser", "멀티 감정사", "판정에 포함된 멀티 강화 카드 1장당 +15 데미지", 6, "add",
          _enhancement_count_bonus("mult", 15, "chips"), "uncommon"),
]

EDITION_COUNT_JOKERS: List[Joker] = [
    Joker("foil_collector", "포일 수집가", "판정에 포함된 포일 카드 1장당 +20 데미지", 8, "add",
          _edition_count_chip_bonus("foil", 20), "rare"),
    Joker("holo_collector", "홀로 수집가", "판정에 포함된 홀로그래픽 카드 1장당 +8 배율", 8, "add",
          _edition_count_mult_bonus("holographic", 8), "rare"),
    Joker("poly_collector", "폴리 수집가", "판정에 포함된 폴리크롬 카드 1장당 배율 x1.15 (최대 x2)", 9, "mult_x",
...
BESPOKE_JOKERS: List[Joker] = [
    Joker("joker_horde", "군중 심리", "보유한 유물 1개당(본인 포함) +1 배율", 5, "add", _joker_horde, "common"),
    Joker("consumable_hoarder", "수집벽", "보유한 보급품 1개당 +8 데미지", 5, "add", _consumable_hoarder, "common"),
    Joker("voucher_scholar", "제도학자", "보유한 훈련 프로그램 1개당 +2 배율", 7, "add", _voucher_scholar, "uncommon"),
    Joker("ante_veteran", "고참병", "현재 스테이지 1당 +2 데미지", 5, "add", _ante_veteran, "common"),
    Joker("blind_slayer_boss", "보스 사냥꾼", "보스 웨이브일 때 +15 배율", 8, "add", _blind_slayer_boss, "rare"),
    Joker("blind_slayer_small", "몸풀기", "1차 웨이브일 때 +10 데미지", 4, "add", _blind_slayer_small, "common"),
    Joker("first_play_bonus", "선공", "이번 웨이브 첫 공격이면 +8 배율", 6, "add", _first_play_bonus, "uncommon"),
    Joker("last_stand", "막판 스퍼트", "이번 공격이 이번 웨이브 마지막 공격이면 배율 x2", 9, "mult_x", _last_stand, "rare"),
    Joker("clean_hands", "깔끔한 손", "이번 웨이브에 재정비를 한 번도 안 했으면 +20 데미지", 6, "add", _clean_hands, "uncommon"),
    Joker("discard_exhausted", "최후의 재정비", "재정비를 모두 소진했으면 +10 배율", 6, "add", _discard_exhausted, "uncommon"),
    Joker("plays_low", "벼랑 끝", "남은 공격이 1회 이하면 +12 데미지", 5, "add", _plays_low, "common"),
    Joker("frugal", "검소함", "보유 자금이 $3 이하면 배율 x1.3", 7, "mult_x", _frugal, "uncommon"),
    Joker("big_spender", "큰손", "보유 자금이 $50 이상이면 +15 배율", 9, "add", _big_spender, "rare"),
    Joker("retrigger_chips", "메아리 손", "점수 카드들의 칩 값을 한 번 더 더함", 9, "add", _retrigger_chips, "rare"),
    Joker("retrigger_mult_small", "겹울림", "콤보의 기본 배율을 한 번 더 더함", 7, "add", _retrigger_mult_small, "uncommon"),
    Joker("solo_act", "독주자", "카드를 1장만 내면 배율 x1.5", 7, "mult_x", _solo_act, "uncommon"),
    Joker("full_hand", "풀핸드", "카드를 5장 다 내면 +40 데미지", 6, "add", _full_hand, "uncommon"),
    Joker("duo_bonus", "듀오", "카드를 정확히 2장 내면 +6 배율", 5, "add", _duo_bonus, "common"),
    Joker("trio_bonus", "트리오", "카드를 정확히 3장 내면 +20 데미지", 5, "add", _trio_bonus, "common"),
    Joker("quartet_bonus", "콰르텟", "카드를 정확히 4장 내면 +9 배율", 6, "add", _quartet_bonus, "uncommon"),
    Joker("omega_joker", "오메가", "이 손패의 칩과 배수를 전부 2배로 만듦", 16, "mult_x", _omega_joker, "legendary"),
    Joker("karma", "인과응보", "보유 자금 $100당 배율 x1.1 (최대 x1.5)", 12, "mult_x", _karma, "legendary"),
    Joker("singularity", "특이점", "점수 카드 수의 제곱만큼 +데미지", 10, "add", _singularity, "rare"),
    Joker("eternal_flame", "영원한 불꽃", "모든 콤보 교범 레벨 합계 1당 배율 x1.05 (최대 x2)", 14, "mult_x", _eternal_flame, "legendary"),
    Joker("lucky_seven", "럭키 세븐", "판정에 7이 포함되면 +21 데미지", 5, "add", _lucky_seven, "common"),
    Joker("even_money", "짝수의 밤", "보유 자금이 짝수면 +6 배율", 4, "add", _even_money, "common"),
    Joker("odd_money", "홀수의 밤", "보유 자금이 홀수면 +10 데미지", 4, "add", _odd_money, "common"),
    Joker("suit_majority", "무늬 지배", "공격에 사용한 카드 중 한 무늬가 3장 이상이면 +10 배율", 6, "add", _suit_majority, "uncommon"),
    Joker("straight_flush_master", "궁극의 조합", "스트레이트 플러시면 +40 배율", 11, "add", _straight_flush_master, "rare"),
    Joker("rare_collector", "감정가", "보유한 레어 이상 유물 1개당 +5 배율", 10, "add", _rare_collector, "rare"),
    Joker("even_purist", "완벽한 짝수", "점수 카드가 전부 짝수면 +16 배율", 7, "add", _even_purist, "uncommon"),
    Joker("odd_purist", "완벽한 홀수", "점수 카드가 전부 홀수면 +50 데미지", 7, "add", _odd_purist, "uncommon"),
    Joker("crimson_hand", "다홍빛 손", "공격에 사용한 카드가 전부 빨강 무늬(♥♦)면 +12 배율", 6, "add", _crimson_hand, "uncommon"),
    Joker("onyx_hand", "흑요석 손", "공격에 사용한 카드가 전부 검정 무늬(♠♣)면 +12 배율", 6, "add", _onyx_hand, "uncommon"),
    Joker("purist", "순수주의자", "점수 카드에 강화가 하나도 없으면 +8 배율", 5, "add", _purist, "common"),
    Joker("edition_lover", "수집가의 눈", "점수 카드 중 에디션이 있으면 배율 x1.4", 10, "mult_x", _edition_lover, "rare"),
    Joker("seal_hunter", "인장 사냥꾼", "점수 카드 중 씰이 있으면 +25 데미지", 7, "add", _seal_hunter, "uncommon"),
    Joker("bicolor", "이색 조합", "공격에 사용한 무늬가 정확히 2종류면 +8 배율", 6, "add", _bicolor, "uncommon"),
    Joker("lowest_two", "최소주의", "낸 카드 중 가장 낮은 카드가 2면 +14 데미지", 4, "add", _lowest_two, "common"),
    Joker("even_count", "짝수 손", "낸 카드 장수가 짝수면 +10 데미지", 4, "add", _even_count, "common"),
    Joker("odd_count", "홀수 손", "낸 카드 장수가 홀수면 +6 배율", 4, "add", _odd_count, "common"),
    Joker("full_slots_bonus", "만원사례", "유물 슬롯이 가득 찼으면 배율 x1.5", 13, "mult_x", _full_slots_bonus, "legendary"),
    Joker("glass_cannon", "유리대포", "점수 카드 중 유리 강화가 있으면 배율 x1.8", 10, "mult_x", _glass_cannon, "rare"),
    Joker("wild_heart", "와일드 하트", "플러시 계열이고 점수 카드 중 와일드 강화가 있으면 +20 배율", 8, "add", _wild_heart, "uncommon"),
    Joker("no_face", "그림 없는 손", "점수 카드에 그림카드(J/Q/K)가 없으면 +10 배율", 5, "add", _no_face, "common"),
    Joker("all_face", "궁정 회의", "점수 카드가 전부 J/Q/K/A면 +18 데미지", 6, "add", _all_face, "uncommon"),
]
...
JOKER_POOL: List[Joker] = [
    Joker("joker_basic", "녹슨 나이프", "+4 배율", 4, "add", _joker_basic, "common"),
    Joker("chip_stacker", "칩 스태커", "+30 데미지", 4, "add", _chip_stacker, "common"),
    Joker("heart_lover", "하트 러버", "공격에 사용한 ♥ 카드 1장당 +3 배율", 5, "add", _heart_lover, "common"),
    Joker("diamond_greed", "그리디", "공격에 사용한 ♦ 카드 1장당 +2 배율", 5, "add", _diamond_greed, "common"),
    Joker("flush_fanatic", "플러시 매니아", "플러시 계열 콤보일 때 카드 1장당 +3 배율", 6, "add", _flush_fanatic, "uncommon"),
    Joker("pair_booster", "페어 부스터", "페어 이상 콤보일 때 +8 배율", 5, "add", _pair_booster, "common"),
    Joker("high_stakes", "하이 스테이크", "5장을 모두 공격에 사용하면 배율 x1.5", 7, "mult_x", _high_stakes, "uncommon"),
    Joker("face_collector", "페이스 수집가", "공격에 사용한 J/Q/K 카드 1장당 +5 데미지", 5, "add", _face_collector, "common"),
    Joker("club_lover", "클럽 마니아", "공격에 사용한 ♣ 카드 1장당 +2 배율", 5, "add", _club_lover, "common"),
    Joker("spade_guardian", "스페이드 수호자", "공격에 사용한 ♠ 카드 1장당 +2 배율", 5, "add", _spade_guardian, "common"),
    Joker("even_lover", "짝수 애호가", "판정에 포함된 짝수 카드 1장당 +1 배율", 4, "add", _even_lover, "common"),
    Joker("odd_lover", "홀수 애호가", "판정에 포함된 홀수 카드 1장당 +1 배율", 4, "add", _odd_lover, "common"),
    Joker("ace_killer", "에이스 킬러", "판정에 에이스가 포함되면 +15 데미지", 5, "add", _ace_killer, "common"),
    Joker("king_pride", "킹의 자존심", "판정에 King이 포함되면 +8 배율", 5, "add", _king_pride, "common"),
    Joker("minimalist", "미니멀리스트", "카드를 1장만 써서 공격하면 +20 데미지", 6, "add", _minimalist, "uncommon"),
    Joker("full_house_lover", "풀하우스 애호가", "풀 하우스일 때 +12 배율", 6, "add", _full_house_lover, "uncommon"),
    Joker("quad_fanatic", "포카드 광신도", "포카드일 때 +20 배율", 9, "add", _quad_fanatic, "rare"),
    Joker("straight_hunter", "스트레이트 사냥꾼", "스트레이트 계열일 때 +10 배율", 6, "add", _straight_hunter, "uncommon"),
    Joker("miser", "구두쇠", "보유 자금 $10당 +1 배율 (최대 +5)", 6, "add", _miser, "uncommon"),
    Joker("pauper", "빚쟁이", "보유 자금이 $5 미만이면 +10 데미지", 4, "add", _pauper, "common"),
    Joker("leisurely", "여유주의자", "남은 재정비 1회당 +2 배율", 6, "add", _leisurely, "uncommon"),
    Joker("passionate", "정열가", "남은 공격 1회당 +3 데미지", 5, "add", _passionate, "common"),
    Joker("collector", "수집가", "콤보 교범 레벨 합계 1당 +2 배율", 8, "add", _collector, "rare"),
    Joker("glass_artisan", "유리공예가", "판정에 포함된 유리 강화 카드 1장당 +10 데미지", 6, "add", _glass_artisan, "uncommon"),
    Joker("wild_dancer", "와일드 댄서", "판정에 포함된 와일드 강화 카드 1장당 +6 배율", 6, "add", _wild_dancer, "uncommon"),
    Joker("golden_hand", "황금손", "카드로 공격할 때마다 +$1", 8, "add", _golden_hand, "rare"),
    Joker("grinder", "노가다꾼", "하이 카드로 낼 때 +10 데미지", 4, "add", _grinder, "common"),
    Joker("two_pair_pro", "투 페어 전문가", "투 페어일 때 +10 배율", 5, "add", _two_pair_pro, "common"),
    Joker("triple_master", "트리플 마스터", "트리플일 때 +12 배율", 6, "add", _triple_master, "uncommon"),
    Joker("almighty", "만능 유물", "판정에 포함된 카드 1장당 +5 데미지, +2 배율", 14, "add", _almighty, "legendary"),
