...
ENHANCEMENT_DESCRIPTIONS = {
    "bonus": "이 카드가 판정에 포함되면 +30 데미지",
    "mult": "이 카드가 판정에 포함되면 +4 배율",
    "wild": "플러시 판정 시 모든 무늬로 취급되며, 나머지 카드가 전부 같은 랭크면 파이브 오브 어 카인드 등 히든 조합의 마지막 한 장을 채워줌",
    "glass": "이 카드가 판정에 포함되면 배율 x2, 이후 25% 확률로 파괴됨",
}

# 카드 코팅 종류와 설명 (강화와 별개로 부여 가능, game.py의 _score_cards에서 처리)
EDITION_DESCRIPTIONS = {
    "foil": "이 카드가 판정에 포함되면 +50 데미지",
    "holographic": "이 카드가 판정에 포함되면 +10 배율",
    "polychrome": "이 카드가 판정에 포함되면 배율 x1.5",
}

# 네거티브 코팅: 플레잉 카드가 아니라 유물/보급품에 붙는 네 번째 코팅.
# 효과는 데미지/배율이 아니라 "슬롯을 차지하지 않음" — 사실상 영구 슬롯 +1.
NEGATIVE_DESCRIPTION = "이 유물/보급품은 보유 슬롯을 차지하지 않습니다."

# 카드 각인 종류와 설명 (강화/코팅과 별개인 세 번째 슬롯, game.py에서 처리)
SEAL_DESCRIPTIONS = {
    "red": "이 카드가 판정에 포함되면 강화·코팅 효과가 한 번 더(재발동) 적용됨",
    "gold": "이 카드로 공격할 때마다 즉시 +$3",
    "blue": "이 카드를 버리면 무작위 보급품 1개를 무료로 획득 (슬롯 여유 시)",
}
...
CHARMS: List[Consumable] = [
    ...
    Consumable("double_charm", "배가 부적", "다음 공격 1회의 배율이 2배가 됩니다.", 6, "charm", _double_charm),
    Consumable("reset_charm", "재정비 부적", "재정비 소모 없이 카드 패를 전부 새로 뽑습니다.", 6, "charm", _reset_charm),
    Consumable("ease_charm", "여유 부적", "이번 웨이브 재정비 횟수가 2회 늘어납니다.", 5, "charm", _ease_charm),
    Consumable("chance_charm", "기회 부적", "이번 웨이브 공격 횟수가 1회 늘어납니다.", 7, "charm", _chance_charm),
]
...
RUNES: List[Consumable] = [
    Consumable(
        ...
        f"{hand_type.label} 교범",
        f"{hand_type.label} 조합의 기본 데미지가 영구히 강화됩니다.",
...
ENHANCERS: List[Consumable] = [
    Consumable(
        f"enhancer_{key}",
        f"{label} 강화 키트",
        f"보유 카드 1장을 개조합니다 — {ENHANCEMENT_DESCRIPTIONS[key]}",
        7,
        "enhancer",
...
EDITIONERS: List[Consumable] = [
    Consumable(
        f"editioner_{key}",
        f"{label} 코팅제",
        f"보유 카드 1장에 특수 코팅을 입힙니다 — {EDITION_DESCRIPTIONS[key]}",
        9,
        "editioner",
...
SPECTRALS: List[Consumable] = [
    Consumable(
        "spectral_mist", "안개", "이번 공격에서 유물로 인한 추가 데미지/배율 효과가 2배로 적용됩니다.",
        8, "spectral", _mist_spectral,
    ),
    Consumable(
        "spectral_echo", "메아리", "이번 웨이브 남은 모든 공격에 +2 배율가 누적 적용됩니다.",
        7, "spectral", _echo_spectral,
    ),
    Consumable(
        "spectral_curse", "저주", "카드 패의 무작위 카드 1장에 무작위 강화를 부여합니다.",
        6, "spectral", _curse_spectral,
    ),
    Consumable(
        "spectral_ruin", "파괴", "지정한 카드 1장을 덱에서 영구히 파괴합니다.",
    ...
    Consumable(
        "spectral_clone", "복제", "보유한 유물 중 하나를 무작위로 복제합니다 (슬롯이 없으면 대신 $10).",
...
SEALERS: List[Consumable] = [
    Consumable(
        f"sealer_{key}",
        f"{label} 각인석",
        f"보유 카드 1장에 각인을 새깁니다 — {SEAL_DESCRIPTIONS[key]}",
        8,
        "sealer",
...
NEGATIVE_EDITIONERS: List[Consumable] = [
    Consumable(
        "editioner_negative_joker", "네거티브 유물석",
        f"보유한 유물 1개를 네거티브 코팅으로 만듭니다 — {NEGATIVE_DESCRIPTION}",
        16, "editioner", _negative_joker_effect, target_type="joker",
    ),
    Consumable(
        "editioner_negative_consumable", "네거티브 보급품석",
        f"보유한 다른 보급품 1개를 네거티브 코팅으로 만듭니다 — {NEGATIVE_DESCRIPTION}",
